...
#include "Abilities/BaseGameplayAbility_Vault.h"
...
UBaseGameplayAbility_Vault::UBaseGameplayAbility_Vault()
{
    NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
}

bool UBaseGameplayAbility_Vault::CommitCheck(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo *ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, OUT FGameplayTagContainer *OptionalRelevantTags)
{
    if(!Super::CommitCheck(Handle, ActorInfo, ActivationInfo, OptionalRelevantTags))
	{
		return false;
	}
	ABaseGASCharacter* Character = GetActionGameCharacterFromActorInfo();
...
void UBaseGameplayAbility_Vault::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo *ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData *TriggerEventData)
{
	...
	MontageTask->OnBlendOut.AddDynamic(this, &UBaseGameplayAbility_Vault::K2_EndAbility);
	MontageTask->OnCompleted.AddDynamic(this, &UBaseGameplayAbility_Vault::K2_EndAbility);
	MontageTask->OnInterrupted.AddDynamic(this, &UBaseGameplayAbility_Vault::K2_EndAbility);
	MontageTask->OnCancelled.AddDynamic(this, &UBaseGameplayAbility_Vault::K2_EndAbility);
	MontageTask->ReadyForActivation();
}

void UBaseGameplayAbility_Vault::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo *ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
